...
<style>
  ...
  /* Premium Scrollbar */
  #galleryContainer::-webkit-scrollbar { height: 4px; }
  #galleryContainer::-webkit-scrollbar-thumb { background: var(--gold); border-radius: 10px; }
  ...
  @media (max-width: 768px) {
    #galleryLeft, #galleryRight { display: none !important; }
  }
  ...
  .alert-error { background-color: #fff5f5; color: #9b2c2c; border: 1px solid #fed7d7; }

  /* Gallery Hover Effect */
  .gallery-thumb.active-thumb { border-color: var(--gold); ring: 2px; --tw-ring-color: var(--gold); }
</style>
...
<div id="bookingModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm hidden items-center justify-center z-[100] p-4">
    ...
    <form id="bookingForm" method="POST" class="p-8 space-y-5">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="col-span-2 sm:col-span-1">
            ...
            <input type="text" id="propertyName" name="propertyName" class="w-full border-gray-100 bg-gray-50 rounded-lg px-3 py-2 text-sm font-semibold" readonly>
        </div>
        <div class="col-span-2 sm:col-span-1">
            <label class="block text-[10px] uppercase tracking-widest font-bold text-gray-500 mb-1">Location</label>
            <input type="text" id="propertyLocation" name="propertyLocation" class="w-full border-gray-100 bg-gray-50 rounded-lg px-3 py-2 text-sm font-semibold" readonly>
        </div>
      </div>

      <div>
        <label class="block text-[10px] uppercase tracking-widest font-bold text-gray-500 mb-1">Full Name</label>
        <input type="text" id="name" name="name" class="w-full border-b-2 border-gray-100 focus:border-gold outline-none py-2 transition" placeholder="John Doe" required>
      </div>

      <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-[10px] uppercase tracking-widest font-bold text-gray-500 mb-1">Phone</label>
            <input type="tel" id="phone" name="phone" class="w-full border-b-2 border-gray-100 focus:border-gold outline-none py-2 transition" placeholder="+91..." required>
          </div>
          <div>
            <label class="block text-[10px] uppercase tracking-widest font-bold text-gray-500 mb-1">Visit Date</label>
            <input type="date" id="visitDate" name="visitDate" class="w-full border-b-2 border-gray-100 focus:border-gold outline-none py-2 transition" required>
          </div>
      </div>

      <div>
        <label class="block text-[10px] uppercase tracking-widest font-bold text-gray-500 mb-1">Email Address</label>
        <input type="email" id="email" name="email" class="w-full border-b-2 border-gray-100 focus:border-gold outline-none py-2 transition" placeholder="example@mail.com" required>
      </div>

      <button type="submit" class="w-full bg-[#0f0f10] text-gold hover:bg-gold hover:text-black font-bold py-4 rounded-xl transition-all uppercase tracking-widest text-sm mt-4 shadow-lg">
        Confirm Request
      </button>
    </form>
  </div>
      ...
      const html = `
        <div class="grid md:grid-cols-2 gap-12 items-start">
          ...
          <div class="flex flex-col h-full">
            ...
            <div class="space-y-4">
                <button id="bookVisitBtn" class="w-full flex items-center justify-center gap-3 bg-[#0f0f10] hover:bg-gray-200 text-white hover:text-black font-bold py-4 rounded-xl transition-all uppercase tracking-widest text-sm shadow-xl">
        ...
        <div class="grid md:grid-cols-2 gap-16 mt-24 items-center">
          <div class="relative">
          ...
          <div>
            ...
            <p class="text-gray-600 mb-8 leading-relaxed font-light text-lg italic border-l-4 border-gold pl-6">${property.description}</p>
